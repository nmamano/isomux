...
Enable **Keep replies until the client acknowledges them** when creating a token for a client that needs reply recovery. The mint API calls this flag `ackMode` and defaults it to `false`. ACK-mode polls retain messages until the client sends `ackThrough`; default polls delete returned messages. Sends and drains accept `Idempotency-Key` for retries within a five-minute in-memory cache. See the [developer API guide](/docs/developer-api) for the request and response fields.
