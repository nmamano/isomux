...
- **New offices start with three welcome agents**, one each for Claude, Codex, and OpenCode. The OpenCode agent can answer immediately without a provider sign-in.
...
- **Built-in safety hooks (Claude and Codex agents)** - blocks `rm -rf`, `git reset --hard`, killing processes created by others, reading secret-bearing files like `.env`, and recognized commands that open outbound tunnels. These guards are an honest-agent safety layer, not OS isolation.
