...
- **Daily verified local backup and restore**: Isomux keeps seven recoverable office backups and verifies each one before it replaces an older copy. Failed or interrupted backups keep the last good copies. Restore is manual. Current backup health is at `GET /api/backup/status`.
