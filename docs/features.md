...
- **Agents use the browser** - Agents can open pages, read, click, fill forms and attach a server file to a file input. Members pair desktop Chrome in Settings → You → Browser Use. The [Chrome extension](self-hosted.md#desktop-chrome-extension) lets members offer an open tab to **All** eligible agents (the default) or one agent with **Agent control** in the popup. Agents read and navigate that tab. Agents can list offered tabs and select an explicit target when several are available. Actions on a shared tab run one at a time. Each new offer defaults to Never expiring. The popup also offers 15 minutes, 1 hour or 4 hours and shows the local expiry time; agent actions do not extend it. Turning control off leaves pages open. Preview cards still run on the server for public and office-hosted pages.
