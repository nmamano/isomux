...
- **Built-in safety checks (Claude, Codex, and OpenCode agents)** - blocks `rm -rf`, `git reset --hard`, killing processes by name or pattern (`pkill`, `killall`), reading secret-bearing files like `.env`, and recognized commands that open outbound tunnels. These guards are an honest-agent safety layer, not OS isolation.
...
- **Daily local backups:** Isomux keeps seven daily office backups. Known credential files are omitted, and each archive includes recovery instructions for what must be entered again.
