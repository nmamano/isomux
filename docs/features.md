...
- **Root-access check on install** — agents run as the server's `isomux` account, so a [VPS install](vps-install.md) checks that the account cannot log in as root, and stops the install if it can or if it cannot tell. The `isomux-harden-ssh` command re-runs the check, and the SSH hardening with it, any time.
- **Out-of-memory protection** — on a [VPS install](vps-install.md), a memory spike costs one agent process instead of the whole box: SSH and the office server are killed last (and Tailscale, if the box uses it), and the agent comes back when you message it.
