...
- **Log secret redaction** - Isomux masks recognized secret patterns before it stores new agent and scheduled-run log entries, including nested tool payloads. Each match keeps its first eight characters plus `...REDACTED`. Existing logs and backend-owned transcripts stay unchanged. Unmatched secrets can still be stored.
