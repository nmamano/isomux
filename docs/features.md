...
- **Managed environment variables** - keep API tokens and other secrets out of prompts: edit office-wide and personal variables in User Settings → Connections, and Isomux stores them in private files under `~/.isomux/`. Personal values override office-wide values. Other per-user variables work the same way, for example `GH_TOKEN` so the `gh` CLI acts as you. Values are not embedded in prompts or conversation logs.
