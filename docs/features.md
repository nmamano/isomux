...
- **Bundled skills** like `/grill-me` (based on the original by Matt Pocock), `/isomux-peer-review`, `/isomux-pair-programming`, `/isomux-soft-handoff`, `/isomux-second-opinion`, `/subagent-review`, `/isomux-report-bug` — available to every agent out of the box
