...
- **Agents use the browser** - Agents can open pages, read, click and fill forms. Each member selects **Server browser** or **Desktop Chrome** in Settings → You → Browser. The [Chrome extension](self-hosted.md#desktop-chrome-extension) gives each agent its own tab in its manager's browser. The popup shows ownership and can stop control, leaving pages open and music playing. Chrome mode has no remote Browser panel and never falls back to the server while offline. In Server browser mode, the office stores one profile per member and shares its logins with that member's agents. The experimental Browser panel is off by default; an owner can enable it in Office Settings. Room members can watch; the manager can interact. The headless page closes after 15 idle minutes unless its manager is watching.
