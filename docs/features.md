...
- **Other bundled skills**: `/grill-me` (based on the original by Matt Pocock), `/handoff` (continue an unfinished task on a fresh session: the agent writes a short brief of what's left, you approve it, and it restarts clean on just that brief), `/wrap-session` (check for loose ends and close a session cleanly), `/isomux-report-bug`.
...
- **Agents use the browser** - Agents can open pages, read, click, fill forms and attach a server file to a file input. Each member selects **Server browser** or **Desktop Chrome** in Settings → You → Browser Use. The [Chrome extension](self-hosted.md#desktop-chrome-extension) lets members offer an open tab to an agent with **Allow agent control** in the popup. The agent reads and navigates that tab. Each new offer defaults to Never expiring. The popup also offers 15 minutes, 1 hour or 4 hours and shows the local expiry time; agent actions do not extend it. Turning control off leaves pages open and music playing. Chrome mode has no remote Browser panel and never falls back to the server while offline. In Server browser mode, the office stores one profile per member and shares its logins with that member's agents. The experimental Browser panel is off by default; an owner can enable it in Office Settings. Room members can watch; the manager can interact. The headless page closes after 15 idle minutes unless its manager is watching.
