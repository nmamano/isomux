...
- **Update notice** — the office header shows when a new release is out. On a [VPS install](vps-install.md), the owner can apply it from there; the confirm step shows how many busy agents the restart would interrupt, and a failed update rolls back on its own. On a source checkout, the notice tracks the isomux repo instead, with steps to pull and restart.
