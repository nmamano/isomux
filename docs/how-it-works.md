...
Isomux uses each backend's own login state. Claude and Codex can use their existing CLI sign-ins. OpenCode uses a shared profile for each environment and supports its providers' API-key login.
