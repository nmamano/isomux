import { mkdirSync, copyFileSync, rmSync, readFileSync, writeFileSync, renameSync } from "node:fs";
import { resolve } from "node:path";
import { extensionZip } from "./extension-zip";

export const EXTENSION_ENTRIES = ["background.js", "connection.css", "connection.html", "connection.js", "manifest.json"] as const;
export async function buildBrowserExtension(
  ...
  const result = await Bun.build({
    entrypoints: [resolve("browser-extension/background.ts"), resolve("browser-extension/connection.ts")],
    outdir,
    target: "browser",
  });
  ...
  try {
    writeFileSync(temporary, extensionZip(EXTENSION_ENTRIES.map(name => ({ name, data: readFileSync(resolve(outdir, name)) }))));
