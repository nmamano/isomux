...
try {
  ...
  try {
    ...
    console.log(
      "JavaScript dialog handler returned success; inspect native evidence.",
    );
  } catch (error) {
  ...
  const sheet = Bun.spawn(
    [
      "ffmpeg",
      "-y",
      ...stages.flatMap((name) => ["-i", join(dir, name + ".png")]),
      "-filter_complex",
      "hstack=inputs=3",
      "-frames:v",
      "1",
  ...
  if ((await sheet.exited) !== 0)
    throw Error("Native evidence contact sheet failed");
  console.log(
    "Native stages.png order: submitted, page-click, javascript-dialog-handler",
  );
