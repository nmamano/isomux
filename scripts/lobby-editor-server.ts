...
const SAVE_DIR =
  process.env.LOBBY_EDITOR_SAVE_DIR ??
  process.env.ISOMUX_APP_DATA_DIR ??
  join(OUT, "saved");
...
function build(): string {
  const r = Bun.spawnSync({
    cmd: [
      "bun",
      "build",
      "ui/office/lobby/preview-entry.tsx",
      "--outdir",
      OUT,
      "--production",
    ],
  ...
  async fetch(req) {
    ...
    if (url.pathname === "/")
      return Response.redirect(
        "/preview.html?edit=1&layout=fireside&mode=light",
        302,
      );
    ...
    if (url.pathname === "/saved") {
      const names = [...new Bun.Glob("*.json").scanSync(SAVE_DIR)]
    ...
    if (url.pathname.startsWith("/saved/")) {
      const name = decodeURIComponent(
        url.pathname.slice("/saved/".length),
...
console.log(
  `lobby editor on ${process.env.ISOMUX_APP_HOST ?? "default host"}:${process.env.PORT ?? 9879}, saves in ${SAVE_DIR}`,
);
