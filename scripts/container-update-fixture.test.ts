...
test("acceptance client sends HTTP requests and reads Bun cookie headers", async () => {
  ...
  const server = Bun.serve({ port: 0, async fetch(request) {
    ...
    expect(request.headers.get("origin")).toBe("https://fixture.example.invalid");
    return new Response("ok", { headers: { "Set-Cookie": "fixture=owner; HttpOnly" } });
  } });
  try {
    const client = Bun.spawn(["python3", "-B", "-c", `
...
assert body == b"ok"
`, new URL("./check-container-update-fixture.py", import.meta.url).pathname, String(server.port)], { stderr: "pipe" });
    const [code, err] = await Promise.all([client.exited, new Response(client.stderr).text()]);
