...
const server = Bun.serve({
  ...
  async fetch(request) {
    ...
    const path =
      pathname.startsWith("/docs/") && !pathname.endsWith(".css")
        ? `site${pathname.replace(/\/$/, "")}/index.html`
        : `site${pathname}`;
...
try {
  for (const viewport of [
    ...
    await context.route("**/*", async (route) => {
      ...
      if (new URL(route.request().url()).pathname === "/api/chat") {
        return route.fulfill({
          ...
          body: '{"error":{"message":"Local verification only"}}',
        });
      }
    ...
    for (const guide of HOSTING_GUIDES) {
      ...
      check(
        (await page.locator(".hosting-flow, .hosting-selector").count()) === 0,
        "guide has no picker",
      );
      check(
        await page.evaluate(
      ...
      if (guide.id === "aws" || guide.id === "private")
        await page.screenshot({
          path: `${output}/${guide.id}-${viewport.width}.png`,
          fullPage: true,
        });
      ...
      await page.evaluate(() => {
        ...
        widget.__chatOpen();
        widget.__chatSend("verification");
      });
      const posted = (await request).postDataJSON() as { pageContext: string };
      const expected = readFileSync(
        `site/_agent/docs/hosting-${guide.id}/index.md`,
        "utf8",
      ).split("\n")[0];
      check(
        posted.pageContext.split("\n")[0] === expected,
  ...
  await noJs.close();
  console.log(
    `PASS: desktop/mobile selection, keyboard, back/forward, legacy links, no JS, context; artifacts ${output}`,
  );
} finally {
  await browser.close();
  await server.stop();
