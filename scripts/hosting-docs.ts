...
export const HOSTING_GUIDES = [
  { id: "hosted", label: "Hosted Isomux", detail: "A paid Isomux hosting service by the creator of Isomux", source: "docs/hosting/hosted.md" },
  { id: "render", label: "Render", detail: "A paid container service", source: "docs/hosting/render.md", notice: "Deployment validation is incomplete. Render charges for the compute plan and persistent disk." },
  { id: "aws", label: "AWS EC2 container", detail: "An Ubuntu server in your AWS account", source: "deploy/container/README.md" },
  { id: "vps", label: "A VPS provider such as Hetzner", detail: "Your cloud server and domain", source: "docs/hosting/vps.md" },
  { id: "local", label: "Host locally", detail: "An office on this computer", source: "docs/hosting/local.md" },
  { id: "private", label: "Private Tailscale", detail: "Your Linux computer, private HTTPS", source: "docs/hosting/private.md" },
  { id: "funnel", label: "Public Tailscale Funnel", detail: "Your Linux computer, no domain needed", source: "docs/hosting/funnel.md" },
  { id: "domain", label: "Your computer and domain", detail: "Your Linux computer, public HTTPS and app addresses", source: "docs/hosting/domain.md" },
] as const;

export type HostingId = (typeof HOSTING_GUIDES)[number]["id"];
export const hostingUrl = (id: HostingId) => `/docs/hosting-${id}`;

const BLOCKS = ["install", "claim", "provider", "service", "tailscale-install", "remote-signin", "invites", "manual-operations", "host-update", "backup"];
export function composeHostingSource(source: string): string {
  return source.replace(/<!-- include: ([\w-]+) -->/g, (_marker, name: string) => {
    if (!BLOCKS.includes(name)) throw new Error(`Unknown hosting block: ${name}`);
    const block = readFileSync(`docs/hosting/blocks/${name}.md`, "utf8").trim();
    if (block.includes("<!-- include:")) throw new Error(`Nested hosting block: ${name}`);
    return block;
  });
}
...
export function hostingBody(guide: (typeof HOSTING_GUIDES)[number]): string {
  ...
  if (guide.id === "aws") raw = raw.replaceAll(
    "](reference.md)", "](https://github.com/nmamano/isomux/blob/main/deploy/container/reference.md)",
  );
  if ("notice" in guide) raw = raw.replace(/^(# .+\n)/, `$1\n> ${guide.notice}\n`);
  return raw;
}
...
function guideLink(id: HostingId, current?: HostingId, connectionLabel?: string): string {
...
export function hostingNavigation(current?: HostingId): string {
  ...
  if (selected) return `<p class="hosting-guide-notice">This is the ${escape(selected.label)} guide. To check if this is the right hosting setup for you, check the <a href="/docs/self-hosted">decision diagram</a>.</p>`;
...
export const HOSTING_LEGACY_LINKS: Record<string, string> = {
  ...
  "desktop-chrome-extension": "/docs/hosting-reference#desktop-chrome-extension",
  ...
  "what-each-deployment-covers": "/docs/hosting-reference#what-each-deployment-covers",
  "proxy-and-real-domain": "/docs/hosting-reference#proxy-and-real-domain",
  "proxy-and-no-real-domain": "/docs/hosting-reference#proxy-and-no-real-domain",
  "no-proxy-and-no-real-domain": "/docs/hosting-reference#no-proxy-and-no-real-domain",
};

export function legacyHostingHtml(): string {
  return `<details class="hosting-legacy"><summary>Links from the previous hosting guide</summary><ul>${Object.entries(HOSTING_LEGACY_LINKS).map(([id, href]) => `<li id="${id}"><a href="${href}">${escape(id.replaceAll("-", " "))}</a></li>`).join("")}</ul></details>
...
export function legacyHostingMarkdown(): string {
  return "\n## Previous section links\n\n" + Object.entries(HOSTING_LEGACY_LINKS).map(([id, href]) => `- [${id}](${href})`).join("\n") + "\n";
}
