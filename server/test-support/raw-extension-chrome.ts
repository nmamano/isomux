...
export async function launchRawExtensionChrome(dir: string) {
  ...
  try {
      ...
      const browser = await chromium.connectOverCDP(adminTransport, { noDefaults: true, timeout: 5000 });
      return { browser, async close() { await browser.close(); chrome?.kill(); if (chrome) await chrome.exited; } };
  } catch (error) { chrome?.kill(); if (chrome) await chrome.exited; throw error; }
}
