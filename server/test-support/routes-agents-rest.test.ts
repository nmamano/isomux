...
// The preview must compose draft fields without persisting any of them.
describe("agents.previewSystemPrompt REST", () => {
  it("previews edits, cleared memory, and a new agent without saving", async () => {
    const srv = await startTestServer();
    server = srv;
    const owner = await srv.seedOwner("Boss");
    const roomId = srv.agentManager.getRooms()[0].id;
    ...
    expect(text).toContain("Original manager");
    expect(text).toContain("Draft name sentinel");
    expect(text).toContain("Draft instructions sentinel");
  ...
  it("rejects inaccessible rooms, mismatched agents, and malformed draft fields", async () => {
    const srv = await startTestServer();
    server = srv;
    const owner = await srv.seedOwner("Boss");
    const member = await srv.seedMember("Mia");
