...
import { anAgentToken, signIn, raw, OFFICE_HOST, HTTPS_ORIGIN, wsConnect, withAppCookie } from "./app-host-test-kit.ts";
...
test("office API starts a real app, authenticates its hostname, relays WS, and preserves it on office restart", async () => {
  ...
  daemon = spawn("python3", [fileURLToPath(new URL("../../deploy/render/supervisor.py", import.meta.url)), "serve", join(root, "runtime")], { stdio: "ignore" });
  const deadline = Date.now() + 5000;
  for (;;) {
    try { adapter.reloadUnits(); break; } catch {
      if (Date.now() > deadline) throw new Error("container supervisor did not start");
      await Bun.sleep(30);
    }
  }
  office = await startTestServer({ startServer: { appSupervisor: adapter } });
  const owner = await office.seedOwner("Boss");
  const configured = await office.http("/api/office/access", { method: "PUT", rawSessionId: owner.rawSessionId,
    headers: { "Content-Type": "application/json" }, body: JSON.stringify({ externalAccess: true, publicOrigin: HTTPS_ORIGIN }) });
  expect(configured.status).toBe(200);
  office = await office.restart();
  expect(buildPublicOrigin().origin).toBe(HTTPS_ORIGIN);
  writeFileSync(join(root, "web.ts"), `
...
`);
  ...
  const registered = await office.http("/api/apps", { method: "POST",
    headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
    body: JSON.stringify({ name: "generated", cwd: root, command: `${process.execPath} web.ts` }) });
  ...
  let read = await raw(office.port, { host: `generated.${OFFICE_HOST}`, headers: withAppCookie(cookie) });
  const readyAt = Date.now() + 3000;
  while (read.status !== 200 && Date.now() < readyAt) {
    await Bun.sleep(50);
    read = await raw(office.port, { host: `generated.${OFFICE_HOST}`, headers: withAppCookie(cookie) });
  }
  ...
  const ws = await wsConnect(office.port, { host: `generated.${OFFICE_HOST}`, path: "/echo", cookie,
    headers: { Origin: `https://generated.${OFFICE_HOST}` } });
  expect(ws.ok).toBe(true);
  if (ws.ok) {
    try {
      ws.client.send("synthetic-echo");
      expect(await ws.client.next()).toEqual({ kind: "text", text: "synthetic-echo" });
    } finally { ws.client.drop(); }
  }
  office = await office.restart();
  const renewedCookie = await signIn(office, "generated", owner.rawSessionId);
  const afterRestart = await raw(office.port, { host: `generated.${OFFICE_HOST}`, headers: withAppCookie(renewedCookie) });
  expect(afterRestart.status).toBe(200);
  expect((JSON.parse(afterRestart.body) as { pid: number }).pid).toBe(pid);
  const deleted = await office.http("/api/apps/generated", { method: "DELETE", rawSessionId: owner.rawSessionId });
  expect(deleted.status).toBe(204);
  const missing = await raw(office.port, { host: `generated.${OFFICE_HOST}`, headers: withAppCookie(cookie) });
