...
it("pushes a full fresh snapshot before auth guidance without rereading Codex", async () => {
  ...
  while (
    !(socket.messages.slice(before) as ServerMessage[]).some(
      (m) => m.type === "log_entry" && m.entry.metadata?.providerLogin,
    ) &&
    Date.now() < deadline
  )
    await Bun.sleep(5);
  const updates = socket.messages.slice(before) as ServerMessage[];
  const freshIndex = updates.findIndex(
    (m) => m.type === "provider_accounts_updated",
  );
  const guidanceIndex = updates.findIndex(
    (m) =>
      m.type === "log_entry" && m.entry.metadata?.providerLogin === "claude",
  );
  expect(freshIndex).toBeGreaterThanOrEqual(0);
  expect(guidanceIndex).toBeGreaterThan(freshIndex);
  const update = updates[freshIndex];
  if (update.type !== "provider_accounts_updated")
    throw new Error("Missing account snapshot");
  expect(update.accounts).toHaveLength(4);
  expect(
    update.accounts.find((a) => a.provider === "claude" && a.scope === "office")
      ?.accountStatus,
  ).toBe("not_connected");
  expect(
    update.accounts.find((a) => a.provider === "codex" && a.scope === "office")
      ?.accountStatus,
  ).toBe("connected");
