...
describe("live browser profile authorization", () => {
  ...
  it("checks room access again at every frame fan-out", async () => {
    ...
    try {
      ...
      await pingPong(socket);
      expect(
        bag(socket).filter((m) => m.type === "browser_frame"),
      ).toHaveLength(1);
  ...
  it("checks the manager on each pointer, key, and navigation event", async () => {
    ...
    try {
      ...
      otherSocket.send({
        type: "browser_input",
        agentId: agent.id,
        input: { kind: "navigate", action: "goto", url: "file:///etc/passwd" },
      });
      otherSocket.send({
        type: "browser_input",
        agentId: agent.id,
      ...
      browserPool.humanInput = originalInput;
  ...
  it("sends page-created notifications only to the manager", async () => {
    ...
    try {
      ...
      await pingPong(otherSocket);
      expect(
        bag(managerSocket).filter((m) => m.type === "browser_action"),
      ).toEqual([{ type: "browser_action", agentId: agent.id }]);
      expect(
        bag(otherSocket).filter((m) => m.type === "browser_action"),
      ).toEqual([]);
      expect("createdPage" in result).toBe(false);
    } finally {
      browserPool.run = original;
    }
  });

  for (const managing of [true, false])
    it(`sends ${managing ? "the full URL to the manager" : "only origin and path to another room viewer"}`, async () => {
      server = await boot();
      const roomId = server.agentManager.getRooms()[0].id;
      const manager = await server.seedOwner("Boss");
      const member = await server.seedMember("Mia");
      await setAccess(server, manager.rawSessionId, member.username, [roomId]);
      const agent = await spawnIn(server, "Managed", roomId, manager);
      const socket = await connectSettled(
        server,
        (managing ? manager : member).rawSessionId,
      );
      const originalWatch = browserPool.watch.bind(browserPool),
        originalStatus = browserPool.status.bind(browserPool);
      const url =
        "https://example.test/login?token=secret-query#secret-fragment";
      ...
      try {
        ...
        browserPool.status = originalStatus;
      }
    });
});
...
describe("room_pet_updated room-ACL (emitted through the registry, not the bridge)", () => {
  it("a pet change reaches a member restricted to that room, and not one who cannot see it", async () => {
    ...
    expect(
      server.agentManager.setRoomPet(r2, { species: "dog", coat: 0 }),
    ).toBe(true);
    expect(
      server.agentManager.setRoomPet(r1, { species: "rabbit", coat: 0 }),
    ).toBe(true);
    ...
    expect((visible as { pet?: { species?: string } }).pet?.species).toBe(
      "rabbit",
    );
    expect(
      bag(memberSock).some(
