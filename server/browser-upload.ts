...
export interface UploadedFile { name: string; mimeType: string; size: number }
export function validUploadPath(path: unknown): path is string {
  return typeof path === "string" && path.length > 0 && path.length <= MAX_BROWSER_UPLOAD_PATH &&
    isAbsolute(path) && !path.includes("\0");
}
function checkPath(path: string): void {
  if (evaluateProposedAction({ kind: "read-files", toolName: "Read", input: { file_path: path } }).decision === "deny")
    throw new BrowserUploadError("Sensitive files cannot be attached");
}
export async function readBrowserUpload(path: string) {
  if (!validUploadPath(path)) throw new BrowserUploadError("path must be an absolute office-server file path (max 4096 characters)");
  ...
  try {
    ...
    try {
      ...
      if (!stat.isFile()) throw new BrowserUploadError("path must name a regular file");
      if (stat.size > MAX_BROWSER_UPLOAD_BYTES) throw new BrowserUploadError("File exceeds the 4 MiB attachment limit");
      ...
      if (size > stat.size || size > MAX_BROWSER_UPLOAD_BYTES)
        throw new BrowserUploadError("File changed or exceeds the 4 MiB attachment limit");
      const name = basename(path).replace(/[\p{Cc}\u202a-\u202e\u2066-\u2069/\\]/gu, "").slice(0, 255) || "attachment";
