...
test.skipIf(process.env.ISOMUX_TEST_BROWSER_EXTENSION !== "1")(
  ...
  async () => {
    ...
    try {
      ...
      await memberRequest(office, owner, "PATCH", "/api/me/browser", {
        backend: "extension",
      });
      expect(
      ...
      const { code } = await (
        await memberRequest(office, owner, "POST", "/api/me/browser/pair", {})
      ).json();
      ...
      await config.evaluate(
        ...
        {
          url:
            buildPublicOrigin().origin.replace("http:", "ws:") +
      ...
      expect((await action(first.id, { action: "close" })).body.closed).toBe(
        true,
      );
      ...
      expect(
        (await memberRequest(office, owner, "DELETE", "/api/me/browser"))
          .status,
      ).toBe(204);
      expect(
