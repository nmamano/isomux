...
test.skipIf(process.env.ISOMUX_TEST_BROWSER_DIALOGS !== "1")(
  ...
  async () => {
    ...
    const [raw] = await Promise.all([launchRawExtensionChrome(dir), buildBrowserExtension(join(dir,"extension"))]);
    let agent: Awaited<ReturnType<typeof chromium.connectOverCDP>> | undefined;
    const nativePicker = async () => {
      const deadline = Date.now() + 500;
      for (;;) {
        const probe = Bun.spawn(["xwininfo", "-root", "-tree"], {stdout:"pipe",stderr:"pipe"});
        ...
        if (Date.now() >= deadline) throw Error("Native window observation failed: "+diagnostic);
        await Bun.sleep(20);
      }
    };
    ...
    const input = '<input id="file" type="file"><output></output><script>file.onchange=()=>document.querySelector("output").textContent=file.files[0]?.name</script>';
    const frames = Bun.serve({hostname:"127.0.0.1",port:0,fetch() {
      return new Response(input,{headers:{"Content-Type":"text/html"}});
    }});
    try {
      ...
      await config.evaluate(async (connection) => {
        const extension = (globalThis as unknown as {chrome:{storage:{local:{set(v:unknown):Promise<void>}}}}).chrome;
      ...
      const target = (await offeredCDP.send("Target.getTargetInfo")).targetInfo.targetId;
      ...
      agent = await chromium.connectOverCDP(browserExtensionTransport(fixture.bridge.forMember("fixture-member")!,"fixture-agent"),{noDefaults:true,timeout:5000});
      ...
      await unrelated.locator("#file").setInputFiles({name:"fake.txt",mimeType:"text/plain",buffer:Buffer.from("fixture")});
      ...
      const upload = async (scope: Page | Frame, label: string) => {
        ...
        await scope.locator("#file").setInputFiles({name:"fake.txt",mimeType:"text/plain",buffer:Buffer.from("fixture")});
      ...
      for(const role of ["root","popup"]) {
        let current = root;
        if (role === "popup") {
          const childPromise = ownedContext.waitForEvent("page",{timeout:5000});
          await root.evaluate((url)=>{window.open(url,"_blank");},frames.url.href);
