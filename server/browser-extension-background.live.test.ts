...
test.skipIf(process.env.ISOMUX_TEST_BROWSER_EXTENSION !== "1")(
  ...
  async () => {
    ...
    try {
              ...
              chrome: {
                tabs: {
                  query(
                    ...
                    Array<{ id: number; active: boolean; windowId: number }>
                  >;
                };
                debugger: {
                  getTargets(): Promise<Array<{ id: string; tabId?: number }>>;
                };
              };
            }
          ).chrome;
      ...
      const activeTabs = (value: Awaited<ReturnType<typeof inventory>>) =>
        value.tabs
          .filter((t) => t.active)
          .map((t) => ({ id: t.id, windowId: t.windowId }));
      ...
      const offeredTarget = (await offeredCDP.send("Target.getTargetInfo"))
        .targetInfo.targetId;
      ...
      for (const role of ["root", "popup"] as const) {
        ...
        await page.evaluate((role) => {
          ...
          document.querySelector("#apply")!.addEventListener("click", () => {
            document.body.dataset.clicks = String(
              Number(document.body.dataset.clicks) + 1,
            );
          });
        }, role);
        ...
        const target = targets.targetInfos.find((t) =>
          role === "root" ? t.targetId === rootId : t.targetId !== rootId,
        )!;
        expect(
          (
            await admin.send("Target.getTargetInfo", {
              targetId: target.targetId,
            })
          ).targetInfo.url,
        ).toBe(page.url());
        const before = await inventory();
        const tabId = before.targets.find(
        ...
        if (role === "root") {
          ...
          const beforeEnable = JSON.parse(snapshot!.state.result.value) as {
            visibility: string;
            focused: boolean;
          };
        ...
        evidence.push({
          ...
          clicks,
          error,
          activeBefore: activeTabs(before),
          activeAfter: activeTabs(after),
        });
      ...
      const retained = setup
        .pages()
        .filter((p) => p.url() === fixture.origin + "/form");
      ...
      expect(
        taskTabIds.every((id) => afterDetach.tabs.some((tab) => tab.id === id)),
      ).toBe(true);
