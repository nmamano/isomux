...
export function resolveBrowserFrame(root: Frame, path: number[]): Frame {
  ...
  for (const index of path) {
    ...
    if (!child || child.isDetached())
      throw new Error("framePath is no longer available; read snapshot or text again");
    frame = child;
  }
  return frame;
}
...
export async function readBrowserFrames(
  ...
  for (const { frame, path } of entries) {
    ...
    try {
      ...
      const options = { timeout: path.length ? Math.min(1500, remaining) : remaining };
      const body = frame.locator("body");
      append(action === "text" ? await body.innerText(options) : await body.ariaSnapshot(options));
    } catch (error) {
