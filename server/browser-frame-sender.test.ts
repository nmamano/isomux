...
function fixture() {
  let buffered = 0, allowed = true;
  const sent: string[] = [];
  const sender = new BrowserFrameSender({send: data => sent.push(data), getBufferedAmount: () => buffered}, () => allowed);
  return { sender, sent, buffer: (value: number) => { buffered = value; }, revoke: () => { allowed = false; } };
}
...
test("a slow socket does not delay a different watcher", () => {
  ...
  slow.buffer(100);
  slow.sender.send("frame"); fast.sender.send("frame");
...
test("access lost while buffered prevents delivery after drain", () => {
  const f = fixture();
  f.buffer(100); f.sender.send("private frame");
  f.revoke(); f.buffer(0); f.sender.flush();
  expect(f.sent).toEqual([]);
});

test("unsubscribe discards the held frame", () => {
  const f = fixture();
  f.buffer(100); f.sender.send("frame"); f.sender.stop();
  f.buffer(0); f.sender.flush(); f.sender.send("later");
  expect(f.sent).toEqual([]);
});
