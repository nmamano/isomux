...
const SECRET_PATTERN = /sk-proj-[A-Za-z0-9_-]{20,}|sk-ant-api03-[A-Za-z0-9_-]{20,}|sk-[A-Za-z0-9]{20,}|m0-[A-Za-z0-9_-]{20,}|ghp_[A-Za-z0-9]{36}|gho_[A-Za-z0-9]{36}|github_pat_[A-Za-z0-9_]{82}|AKIA[0-9A-Z]{16}|(?i:(api[_-]?key|secret|token|password)\s*[=:]\s*['"]?[A-Za-z0-9_\-+/.=]{16,})/g;
...
export function redactLogEntry(entry: LogEntry): LogEntry {
  ...
  function copy(value: unknown): unknown {
    if (typeof value === "string") {
      return value.replace(SECRET_PATTERN, (match) => match.slice(0, 8) + "...REDACTED");
    }
  ...
  while (pending.length > 0) {
    ...
    for (const [key, child] of Object.entries(source)) {
      Object.defineProperty(target, key, {
        value: copy(child), enumerable: true, configurable: true, writable: true,
      });
    }
  }
  return result;
