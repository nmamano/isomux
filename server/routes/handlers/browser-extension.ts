...
export function browserExtensionHandlers(service: BrowserExtensionService, select: (member: string, backend: BrowserBackend) => Promise<void>): Record<string, RouteHandler> {
  return {
    "browser.get": (ctx) => ok(service.status(ctx.identity.userId!)),
    "browser.select": async (ctx) => {
      const body = ctx.body as { backend?: unknown } | null;
      if (!body || (body.backend !== "headless" && body.backend !== "extension")) return fail(422, "invalid_request", "backend must be headless or extension");
