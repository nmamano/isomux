...
function entry(content: string, kind: LogEntry["kind"] = "tool_result"): LogEntry {
  return { id: "log-test", agentId: "agent-test", timestamp: 123, kind, content };
}
...
describe("log secret redaction", () => {
  ...
  for (const secret of secrets) {
    it("redacts exact pattern " + secret.split(/[=:]/)[0].slice(0, 12), () => {
      expect(redactLogEntry(entry("before " + secret + " after")).content)
        .toBe("before " + redacted(secret) + " after");
    });
  ...
  it("scans all kinds, nested arrays, custom metadata and tool payloads without mutation", () => {
    const kinds: LogEntry["kind"][] = ["tool_result", "tool_call", "text", "thinking",
      "system", "error", "user_message", "api_token_outbound", "diff",
      "edit-request", "terminal-command", "file-view"];
    for (const kind of kinds) {
      const input = { ...entry(secrets[0], kind),
        metadata: { payload: [{ command: secrets[1], count: 7, flag: false, nil: null }], [secrets[2]]: "key is structural" },
        terminal: { command: secrets[3] },
      };
      const result = redactLogEntry(input);
      expect(result.content).toBe(redacted(secrets[0]));
      expect(result.metadata).toEqual({ payload: [{ command: redacted(secrets[1]), count: 7, flag: false, nil: null }], [secrets[2]]: "key is structural" });
  ...
  it("shows the exact generic false-positive surface", () => {
    ...
    expect(redactLogEntry(entry("https://example.com/?token=abcdefghijklmnop&next=docs")).content)
      .toBe("https://example.com/?token=ab...REDACTED&next=docs");
    expect(redactLogEntry(entry('curl https://example.com -d "api_key=YOUR_API_KEY_HERE"')).content)
      .toBe('curl https://example.com -d "api_key=...REDACTED"');
    for (const name of ["DATABASE_PASSWORD", "database_password"]) {
      expect(redactLogEntry(entry(name + "=hunter2hunter2hunter2")).content)
        .toBe(name.slice(0, -8) + name.slice(-8) + "...REDACTED");
    }
    expect(redactLogEntry(entry("password=correct-horse-battery-staple")).content)
...
it("handles 20000 nested objects without call-stack overflow and preserves undefined", () => {
  let metadata: Record<string, unknown> = { value: "sk-" + "x".repeat(20), absent: undefined };
...
it("pins the PM ruling and stays stable on a second pass for every case", () => {
  const cases = [
    ["Environment=OPENAI_API_KEY=sk-proj-" + "A".repeat(24), "Environment=OPENAI_API_KEY=...REDACTED"],
    ["Environment=ANTHROPIC_API_KEY=sk-ant-api03-" + "A".repeat(24), "Environment=ANTHROPIC_API_KEY=...REDACTED"],
    ["DATABASE_PASSWORD=hunter2hunter2hunter2hunter2", "DATABASE_PASSWORD...REDACTED"],
    ["database_password=hunter2hunter2hunter2hunter2", "database_password...REDACTED"],
    ["api_key=YOUR_API_KEY_HERE_PLACEHOLDER", "api_key=...REDACTED"],
    ["https://example.com/v1/things?token=abcdefghijklmnopqrstuvwx", "https://example.com/v1/things?token=ab...REDACTED"],
    ["AKIAIOSFODNN7EXAMPLE", "AKIAIOSF...REDACTED"],
    ["ghp_" + "a".repeat(36), "ghp_aaaa...REDACTED"],
    ['curl -H "Authorization: Bearer $ISOMUX_AGENT_TOKEN"', 'curl -H "Authorization: Bearer $ISOMUX_AGENT_TOKEN"'],
