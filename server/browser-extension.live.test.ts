...
test.skipIf(process.env.ISOMUX_TEST_BROWSER_EXTENSION !== "1")("real extension owns only its task tab and drops disconnected work", async () => {
  ...
  try {
    ...
    const configure = async () => {
      await extensionPage.evaluate(async config => {
        ...
        const extensionChrome = (globalThis as unknown as { chrome: { storage: { local: { set(value: unknown): Promise<void> } } } }).chrome;
    ...
    expect(await page.locator("output").getAttribute("data-trusted")).toBe("true");
    await page.screenshot({ path: join(dir, "task.png") });
    expect(readFileSync(join(dir, "task.png")).subarray(0, 8).toString("hex")).toBe("89504e470d0a1a0a");
    ...
    expect(await context.newPage().then(() => false, () => true)).toBe(true);
    expect(await root.send("Target.getTargetInfo", { targetId: "unrelated" }).then(() => false, () => true)).toBe(true);
    expect(await root.send("Storage.getCookies").then(() => false, () => true)).toBe(true);
    ...
    await configure();
    expect(fixture.bridge.forMember("fixture-member")!.generation).not.toBe(old.generation);
    agent = await connect();
    ...
    expect(setupTargets.targetInfos.filter(target => target.url === fixture.origin + "/form")).toHaveLength(2);
    expect(setupTargets.targetInfos.some(target => target.url === fixture.origin + "/unrelated")).toBe(true);
    const elapsed = performance.now() - started;
    expect(elapsed).toBeLessThan(30_000);
    writeFileSync(join(dir, "evidence.json"), JSON.stringify({
      date: new Date().toISOString(), playwright: "1.62.1", chrome: setup.browser()!.version(),
      durationMs: Math.round(elapsed), assertions: ["real extension loaded", "assigned targets only", "snapshot and text", "fill and trusted mouse click", "PNG screenshot", "pending rejected after observed side effect", "fresh generation; side effect remains one", "detach leaves pages open"],
    }, null, 2));
