...
export class BrowserExtensionStore {
  ...
  constructor(
    ...
    try {
      ...
      if ("version" in raw) {
        ...
        if (
          envelope.version !== 1 ||
          typeof envelope.selectionRequired !== "boolean" ||
          !envelope.members ||
  ...
  redeem(
  ...
  ): { member: string; credential: string } {
    ...
    if (
      ...
      throw new Error("browser_pairing_refused");
    this.codes.delete(hash);
    if (this.record(pending.member).hash && !pending.replace)
      throw new Error("browser_pairing_refused");
    const credential = secret();
    this.write(pending.member, {
      ...this.record(pending.member),
  ...
  memberForHash(hash: string, origin?: string): string | undefined {
    return Object.keys(this.records).find((member) => {
      ...
      return (
        record.hash === hash &&
        (origin === undefined || record.origin === origin)
      );
    });
  }
