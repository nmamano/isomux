...
test("upload schema requires one absolute path and a bounded selector", () => {
  for (const path of [
    ...
    expect(
      parseBrowserParams({ action: "upload", selector: "#file", path }),
    ).toMatchObject({ ok: false, code: "invalid_request" });
  for (const selector of [undefined, "", "x".repeat(20_000)])
    expect(
      parseBrowserParams({ action: "upload", selector, path: "/tmp/file" }),
    ).toMatchObject({ ok: false, code: "invalid_request" });
  expect(
    parseBrowserParams({
      action: "upload",
  ...
  ).toMatchObject({ ok: true, action: "upload" });
});
...
test("file payload is bounded, binary-exact, named and typed without a path", async () => {
  ...
  try {
    ...
    expect(
      await readBrowserUpload(path).then(
        () => false,
        () => true,
      ),
    ).toBe(true);
    ...
    expect(await readBrowserUpload(unknown)).toEqual({
      name: "file.unknown-extension",
      mimeType: "application/octet-stream",
      buffer: Buffer.alloc(0),
    });
    expect(
      await readBrowserUpload(dir).then(
        () => false,
        () => true,
      ),
    ).toBe(true);
    expect(
      await readBrowserUpload(join(dir, "missing")).then(
        () => false,
        () => true,
      ),
    ).toBe(true);
...
test("upload rejects sensitive requested names and resolved symlink targets", async () => {
  ...
  try {
    ...
    for (const name of [
      ...
      expect(
        await readBrowserUpload(alias).then(
          () => false,
          () => true,
        ),
      ).toBe(true);
