...
test("known selector parse failures return fixed guidance without private input", () => {
  ...
  const role = selectorSyntaxFailure(new Error(`click: Error: Unknown attribute "${privateText}", must be one of "name".\nDOM: ${privateText}`));
  const css = selectorSyntaxFailure(new Error(`locator.ariaSnapshot: Unexpected token "${privateText}" while parsing css selector "[".\nDOM: ${privateText}`));
...
test("non-parser failures keep their existing classification", () => {
  const timeout = new Error('page.click: Unexpected token "x" while parsing css selector "["');
  timeout.name = "TimeoutError";
  for (const error of [
    timeout,
    new Error('page.click: strict mode violation: Unknown attribute "exact", must be one of "name"'),
    new Error('page.evaluate: Error: Unknown attribute "exact", must be one of "name"'),
