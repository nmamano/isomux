...
import { parseAllowedEvent } from "./transport.ts";
...
describe("OpenCode OC1 raw-ingress allowlist", () => {
  ...
  it("reduces provider errors to a session id and fixed kind", () => {
    const parsed = parseAllowedEvent(
      JSON.stringify({
        type: "session.error",
        properties: { sessionID: "session-1", error: { message: "PROVIDER_SECRET_CANARY" } },
      }),
    );
    expect(parsed).toEqual({ kind: "error", sessionId: "session-1" });
  });
