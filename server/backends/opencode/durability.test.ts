  ...
  it("adopts the pinned server and resumes context after the Isomux process is SIGKILLed", async () => {
    ...
    const mock = Bun.serve({
      ...
      async fetch(request) {
        ...
        const recalled = messages.some((message) =>
          typeof message.content === "string" && message.content.includes("S4_CONTEXT_CANARY"),
        );
        const text = last === "GATE_RECALL"
          ? recalled ? "RECALLED:S4_CONTEXT_CANARY" : "RECALLED:EMPTY"
          : "FIRST_TURN_STORED";
    ...
    const config = JSON.stringify({
      ...
      provider: {
        gate: {
          ...
          env: [],
          models: { "gate-model": { name: "Gate", limit: { context: 100000, output: 10000 }, cost: { input: 0, output: 0 } } },
    ...
    const run = (phase: "first" | "resume") => Bun.spawn(
    ...
    const prior = JSON.parse(await readFile(resultPath, "utf8")) as { serverPid: number };
    ...
    const health = await fetch(`http://127.0.0.1:${record.port}/global/health`, {
      headers: {
