...
import {
  assertOpenCodeServerStopped,
  preserveOpenCodeAuthProviders,
} from "./login-runner.ts";
...
describe("OpenCode exact-profile authentication install", () => {
  it("restores prior providers after exact-profile login", async () => {
    const root = await mkdtemp(
      join(tmpdir(), "isomux-opencode-auth-preserve-"),
    );
    scratch.push(root);
    ...
    await writeFile(
      before,
      JSON.stringify({ openai: { type: "api", key: "first" } }),
      { mode: 0o600 },
    );
    await writeFile(
    ...
    expect(await Bun.file(join(profile, "server.replace")).text()).toBe(
  ...
  it("refuses exact-profile output that lacks the requested provider", async () => {
    const root = await mkdtemp(
      join(tmpdir(), "isomux-opencode-auth-provider-"),
    );
    scratch.push(root);
    const profile = join(root, "profile");
    const authDir = join(profile, "data", "opencode");
  ...
  it("fails fast when the shared server restarted before login", async () => {
    ...
    await expectRejection(
      assertOpenCodeServerStopped(root),
      /get a fresh login command/,
    );
