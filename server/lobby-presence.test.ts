...
const row = (
  ...
  lobbyMoveAt = 100,
) => ({ connectionId, lobbySpotId, lobbyMoveAt });
...
it("moves due ghosts in sequence into distinct free spots, leaving other deadlines alone", () => {
  ...
  expect(
    planLobbyMoves(before, ["one", "two", "three", "four"], 99, () => 0),
  ).toEqual(before);
  const next = planLobbyMoves(
    before,
    ["one", "two", "three", "four"],
    100,
    () => 0.99,
...
it("keeps a full lobby stable, promotes overflow first, and uses the supplied random source", () => {
  ...
  expect(
    planLobbyMoves(full, ["one", "two"], 100, () => 0).map(
      (p) => p.lobbySpotId,
    ),
  ...
  expect(
    planLobbyMoves([row("a", null)], ["one", "two"], 0, () => 0.99)[0],
  ).toEqual(row("a", "two", 24900));
});
...
it("assigns an entry with injected randomness without moving or dropping waiting rows", () => {
  ...
  expect(
    assignLobbySpot(before, ["one", "two", "three"], "new", 100, () => 0),
  ).toEqual(row("new", null, 15100));
  expect(
    assignLobbySpot(
      before,
      ["one", "two", "three", "four"],
