...
How to use a web page yourself (read it, click it, fill a form): call POST localhost:${PORT}/api/agents/${agentId}/browser with your bearer token and {"action":"..."}. Your manager pairs desktop Chrome in Settings → You → Browser Use. Other chat speakers do not change whose browser you use. Each agent controls its own tab. Actions: "goto" with "url" opens a page; "snapshot" returns the ARIA tree; "text" returns rendered text; "click" and "fill" take a Playwright "selector", and "fill" also takes "text"; "press" takes a "key" and optional "selector"; "screenshot" adds an image to chat; "close" ends control. Each action returns the page's "url" and "title".
