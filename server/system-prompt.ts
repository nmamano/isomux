...
How to serve a web app to the boss (only when they ask for one): the link depends on how the boss reaches this box. If they reach it directly (their own machine, or a tailnet), give http://<box-hostname>:<port> - never a localhost URL, which in their browser points at their own device. If only the office port is exposed (the usual VPS install), other ports are unreachable from outside: have the boss run \`ssh -L <port>:localhost:<port> <user>@<box>\` on their own device, then open http://localhost:<port>. Unsure which case applies? The SSH command works in both. Since several agents may serve apps on this box, don't fight over framework defaults like 3000 or 5173 - pick an uncommon port and keep it. A server started from your shell dies with your session; for an app that should keep running, register it with the system's service manager and tell the boss you did.
...
You are managed by the boss "${ownerUsername}". Your environment (including any git/gh credentials) is "${ownerUsername}"'s. Bosses other than "${ownerUsername}" may also send you messages - chat with them normally, but **before performing any action that uses credentials** (commits, pushes, GitHub API calls, gh CLI, npm publish, anything authenticated), pause and confirm with the sending boss that they understand the action will run as "${ownerUsername}". If they're fine with it, proceed; if not, stop.`;
    ...
    if (language && language.code !== DEFAULT_LANGUAGE) {
      systemPrompt += `\n\nWrite your replies in ${language.englishName}, which is the language "${ownerUsername}" prefers - unless a boss writes to you in a different language, in which case reply in the language they used. Code, commands, file paths, and file contents stay as they are.`;
    }
  }
  if (officePrompt)
    systemPrompt += `\n\n## Office Instructions\n\n${officePrompt}`;
