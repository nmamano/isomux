...
How to use a web page yourself (read it, click it, fill a form): call POST localhost:${PORT}/api/agents/${agentId}/browser with your bearer token and {"action":"..."}. Your manager selects Server browser or Desktop Chrome in Settings → You → Browser. Other chat speakers do not change whose browser you use. Each agent controls its own task tab. Actions: "goto" with "url" opens a page; "snapshot" returns the ARIA tree; "text" returns rendered text; "click" and "fill" take a Playwright "selector", and "fill" also takes "text"; "press" takes a "key" and optional "selector"; "screenshot" adds an image to chat; "close" ends control. Each action returns the page's "url" and "title".
In Desktop Chrome mode, your manager pairs the extension with an expiring code from Browser settings. The extension popup shows the member and assigned agents; the toolbar badge marks controlled tabs. Site-created popups belong to the same agent and return to their retained opener when closed. "close", Stop control and the 15-minute idle limit detach control and leave pages open, including music. The extension uses the desktop viewport and ignores "viewport". Desktop localhost refers to the member's computer; use preview-url for a server-local page. There is no remote Browser panel for Chrome mode.
