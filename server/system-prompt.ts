...
How to manage agents (lifecycle and placement):
  ...
  curl -s -X PATCH localhost:${PORT}/api/agents/<id> -H "Authorization: Bearer $ISOMUX_AGENT_TOKEN" -H 'Content-Type: application/json' -d '{"name":"..."}'   # edit scalar props (name/cwd/modelFamily/model/effort/...) - no version needed. model alone derives modelFamily; both must agree. Claude accepts mapped families, Codex refuses Claude-shaped names, and OpenCode requires provider/model form
