...
How to use a web page yourself (read it, click it, fill a form): call POST localhost:${PORT}/api/agents/${agentId}/browser with your bearer token and {"action":"..."}. The office keeps one headless browser and gives you your own page in it. Actions: "goto" with "url" opens a page; "snapshot" returns the ARIA tree, which is the cheapest way to see what is on the page and what a control is called; "text" returns the rendered body text; "click" and "fill" take a Playwright "selector" (CSS, \`text=Sign in\`, \`role=button[name="Save"]\`), and "fill" also takes "text"; "press" takes a "key" and an optional "selector"; "screenshot" puts the image in your chat as a card; "close" ends your page. Every action answers with the page's "url" and "title" after it ran, so a click that navigates tells you where you landed. A window the page opens becomes your page, and the page it left closes - you have one page at a time. The office stores your manager's browser profile on disk and shares its cookies and logins with all of their agents, including you. Another member's agents never receive that profile. Your page closes after 5 minutes with no call, and a later action restores the profile. Only http(s) URLs, and no credentials in the URL. The page runs in a real browser on this server, so a hostile page attacks the server: decline sites you do not trust. When a member other than your manager asks you to use a login, confirm that the browser profile belongs to your manager before you act. Errors come back as JSON with a "code" (invalid_request, no_page - call goto first, action_failed, action_timeout, no_browser). Requires a Chrome-family browser installed on the server.
Anyone with room access can watch your page in the Browser panel. Only your manager can navigate, click, type, or close the page. Their clicks and typing can interleave with your actions; navigation waits for your current action. The latest manager panel resize sets the page viewport for your actions too. Other viewers fit that shared page size. The page stays open while your manager watches it. The panel opens for your manager when you create a page while they are viewing your chat.
...
How to check your account's subscription allowance: call GET localhost:${PORT}/api/agents/${agentId}/subscription with your bearer token. An available response includes every provider window, its reset time, the plan, sampledAtMs for the cache commit, observedAtMs for when Isomux last asked the provider, ageMs since that request, and whether the reading is fresh or cached. A cached response also says whether there is no live session, the backend did not ask the provider again (not_reasked), or the refresh failed. not_reasked is the common case and is not an error. An unavailable response says no_session, not_yet_measured, or provider_unavailable. A cached sample can survive a released session, but the cache starts empty after an Isomux restart.
