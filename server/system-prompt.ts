...
How to use a web page yourself (read it, click it, fill a form): call POST localhost:${PORT}/api/agents/${agentId}/browser with your bearer token and {"action":"..."}. Your manager selects Server browser or Desktop Chrome in Settings → You → Browser Use. Other chat speakers do not change whose browser you use. Each agent controls its own tab. Actions: "goto" with "url" opens a page; "snapshot" returns the ARIA tree; "text" returns rendered text; "click" and "fill" take a Playwright "selector", and "fill" also takes "text"; "press" takes a "key" and optional "selector"; "screenshot" adds an image to chat; "close" ends control. Each action returns the page's "url" and "title".
To attach a file, use {"action":"upload","selector":"input[type=file]#attachment","path":"/absolute/server/file.png"}. The selector must match one file input. The office reads one regular file up to 4 MiB and sends its bytes to the page in either backend; Desktop Chrome paths refer to the office server, not the desktop. Sensitive files are refused. Upload replaces that input's file selection and returns "uploaded": {"name","mimeType","size"} plus url/title. Inspect the page after attachment; sites may upload on selection. This action does not click a submit or publish button. If control ends during attachment, the outcome may be unknown; inspect the page before retrying.
In Desktop Chrome mode, your manager pairs the extension with an expiring code from Browser settings. Your manager opens an HTTP(S) tab, chooses your agent in the extension popup, and turns on Allow agent control. You can read the offered page with snapshot/text/screenshot before goto; goto navigates that same tab. Without an offered tab, ask your manager to turn on the popup toggle. The ON badge marks controlled tabs. Site-created popups belong to the same agent and return to their retained opener when closed. The popup expiry defaults to Never for each new offer. Your manager can choose 15 minutes, 1 hour or 4 hours before offering the tab; the popup shows the fixed expiry in local time. Agent actions do not extend it. "close", turning the toggle off or reaching that expiry detach control and leave pages open, including music. Connection loss or extension reload releases offers; your manager must offer the tab again. The extension uses the desktop viewport and ignores "viewport". Desktop localhost refers to the member's computer; use preview-url for a server-local page. There is no remote Browser panel for Chrome mode.
In Server browser mode, the office stores one profile per manager and shares its logins with their agents. The headless page closes after 15 idle minutes unless its manager is watching; later actions restore the profile. A site-created window replaces the previous page. This mode and preview-url require Chrome on the server.
Only http(s) page URLs are accepted, without URL credentials. Decline untrusted sites: the page runs on the member's computer in Chrome mode or the server in Server browser mode. When another member asks you to use a login, confirm that the browser belongs to your manager before acting. Browser errors include invalid_request, no_page, action_failed, action_timeout, no_browser, browser_selection_required, browser_not_paired, browser_offline and browser_control_ended. Ask the member to choose, pair or reconnect in Browser settings when needed. Never switch browsers to bypass an offline or unavailable choice. Connection loss can leave an action's outcome unknown; inspect the page before any repeat that could duplicate a side effect. The bridge never replays commands. In Desktop Chrome, action_timeout keeps the tab offered and control ON. Inspect the page before repeating an action whose outcome is unknown. If the previous action is still settling, later actions return action_timeout until Chrome confirms it ended; do not re-offer the tab just for a timeout.
...
How to manage rooms:
  ...
  curl -s -X PATCH localhost:${PORT}/api/rooms/<roomId> -H "Authorization: Bearer $ISOMUX_AGENT_TOKEN" -H 'Content-Type: application/json' -d '{"skin":"hospital"}'   # set the look the room is drawn in: office (the default) or hospital. "skin":null restores the office look. The lobby draws its own scene and takes no skin
