...
import {
  AppSupervisorError, appHostEnvDirective, appUrlEnvDirective,
  computeAppPath, type AppRuntime, type AppSupervisor,
...
export function createContainerAppSupervisor(
  ...
  function rpc<T>(op: string, args: Record<string, unknown> = {}): T {
    const reply = spawnSync("python3", [client, "client", socketPath], {
      input: JSON.stringify({ op, ...args }), encoding: "utf8", timeout: 20_000,
      maxBuffer: 2 * 1024 * 1024,
    });
    // Never include stderr, command text, or token-bearing requests in errors.
    if (reply.error || reply.status !== 0)
      throw new AppSupervisorError("supervisor_failed", "Container supervisor is unavailable");
    let result: { ok: boolean; value?: T; error?: string };
    try { result = JSON.parse(reply.stdout); }
    catch { throw new AppSupervisorError("supervisor_failed", "Invalid supervisor response"); }
    if (!result.ok)
      throw new AppSupervisorError("supervisor_failed", result.error || "Container supervisor refused the operation");
    return result.value as T;
  }
  function definition(app: AppRecord) {
    const url = appPublicUrl(app.hostLabel, domain());
    const env: Record<string, string> = {
      PORT: String(app.port), ISOMUX_APP_NAME: app.name,
    ...
    if (url !== null) {
      ...
      descriptor.push(appUrlEnvDirective(url), appHostEnvDirective("127.0.0.1"));
    }
    return { name: app.name, command: app.command, cwd: app.cwd, env,
