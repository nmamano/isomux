...
test("production context uses committed regular files and excludes private and untracked files", () => {
  ...
  const run = (...args: string[]) => spawnSync(args[0], args.slice(1), { cwd: root, encoding: "utf8" });
  try {
    ...
    expect(run("git", "-c", "user.name=Fixture", "-c", "user.email=fixture@example.invalid", "commit", "-qm", "fixture").status).toBe(0);
    ...
    expect(run("tar", "xOf", "first.tar", "server/main.ts").stdout).toBe("committed");
    expect(run("python3", exporter, "HEAD", "second.tar").status).toBe(0);
    expect(readFileSync(join(root, "first.tar"))).toEqual(readFileSync(join(root, "second.tar")));
    symlinkSync("/outside", join(root, "server/link"));
    expect(run("git", "add", "server/link").status).toBe(0);
    expect(run("git", "-c", "user.name=Fixture", "-c", "user.email=fixture@example.invalid", "commit", "-qm", "link").status).toBe(0);
...
test("legacy Render build has the same image and context contract", () => {
  for (const name of ["Dockerfile", "Dockerfile.dockerignore"]) {
    expect(readFileSync(new URL(`../render/${name}`, import.meta.url), "utf8")).toBe(
