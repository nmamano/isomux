...
2. Unread pointer server-side per user. 5. No sound. 6. Edit in place if clean, else skip. 7. Attachments, same cards as the agent chat: not a second-class chat.
3. Mini ghost next to the author name as a trial; Nil decides on sight. 9. Keep forever, storage page reports it. 10. Name: "Members chat".
4. Order is mine. The scene visuals are not done; do not depend on a final layout.
...
- Demo (ui/demo-server.ts): a canned stream (Ricky, Stephen, an API token line, an agent line) behind GET/POST/PATCH/DELETE /api/members-chat and POST /api/members-chat/read, fanning out members*chat*\* through shimEmit; `bun run build:demo` is a gate this slice.
