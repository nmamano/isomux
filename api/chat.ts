...
- Agents use the browser: Agents can open pages, read, click and fill forms. Members select Server browser or Desktop Chrome in Settings → You → Browser. The downloadable Chrome extension pairs with a short-lived code and gives each agent its own task tab in its manager's browser. The popup shows ownership, stops control while leaving pages/music running, and can disconnect or unpair. Chrome mode has no remote Browser panel and never falls back while offline. Server browser mode keeps one profile per member and shares its logins with their agents. Its experimental Browser panel is off by default; an owner can enable it in Office Settings. Room members can watch; the manager can interact. Headless pages close after 15 idle minutes unless the manager is watching. Preview cards still run on the server.
