...
- Isomux additions: /isomux-all-hands (shows what everyone is up to), /isomux-system-prompt (dumps the full assembled system prompt), /isomux-cronjob-system-prompt (same for cron jobs), /isomux-diff (rich-rendered uncommitted changes in the agent's cwd — agents can also choose to show a diff card on their own), /isomux-edit (open a file in the side-panel editor; agents can offer this on their own too), /isomux-usage (per-agent / per-room / per-cron-job token spend, scoped to the rooms you can access)
...
- Root-access check on install: agents run as the server's isomux account, so a VPS install checks that the account cannot log in as root, and stops the install if it can or if it cannot tell. The isomux-harden-ssh command re-runs the check, and the SSH hardening with it, any time
- Out-of-memory protection: on a VPS install, a memory spike costs one agent process instead of the whole box. SSH and the office server are killed last (and Tailscale, if the box uses it), and the agent comes back when you message it
