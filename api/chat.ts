...
- Personal API tokens: one token has one durable conversation with all the agents it messages; sends and replies share a sequence log, read by cursor in pages of 500 until latestSequence, with polling until push access is available. Only owner-driven storage pruning removes entries, including after revocation. See isomux.com/docs/developer-api for the protocol and Idempotency-Key retries.
