...
- Agents use the browser: An agent can open a page, read it, click a control, and fill a form. The live Browser panel is experimental and off by default. An owner can enable it in Office Settings so room members can watch the page and the agent's manager can navigate, click, type, log in, drag to select text, copy the selection, and close the page. When enabled, the panel opens automatically if the agent creates a page while the manager views that chat. The office stores one browser profile per member and shares its logins with that member's agents. The page closes after 15 idle minutes unless its manager is watching.
