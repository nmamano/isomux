...
- Agents use the browser: An agent can open a page, read it, click a control, and fill a form. Anyone with room access can watch the page in the Browser panel. The agent's manager can also navigate, click, type, log in, and close the page. Opening the panel lets the manager browse without the agent. The page viewport follows the manager panel size; the latest resize wins across their devices. Other viewers fit that shared page size. A narrow panel can select a site’s mobile layout. The panel opens automatically when the agent creates a page while the manager is viewing that chat. The office stores one browser profile per member and shares its logins with that member's agents. The page closes after five idle minutes unless its manager is watching.
