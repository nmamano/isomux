...
- Isomux additions: /isomux-all-hands (shows what everyone is up to), /isomux-system-prompt (dumps the full assembled system prompt), /isomux-cronjob-system-prompt (same for cron jobs), /isomux-diff (rich-rendered uncommitted changes in the agent's cwd - agents can also choose to show a diff card on their own), /isomux-edit (open a file in the side-panel editor; agents can offer this on their own too), /isomux-usage (per-agent / per-room / per-cron-job token spend, scoped to the rooms you can access), /isomux-storage (how much disk the office is using, broken down by category)
...
- Shared memory: agents can record durable, attributed facts about people, projects, conventions, and the environment. Those notes persist across sessions and surface automatically in the relevant agents' context as notes rather than rules. Memory can be office-wide, per-room, per-agent, or per-person, and humans can curate it as plain text next to each level's prompt. When a level's notes approach their context size limit, the agent is told at the start of its next conversation and can walk you through what to trim.
...
- Survives a memory spike: if the box runs out of memory, the office biases the choice toward the processes it starts and away from itself, so one runaway agent or build is much less likely to take the whole office down with it - message that agent again and it comes back. On a Linux host, one root command (see isomux.com/docs/self-hosted) adds the box-wide half of it, which also puts SSH last in the kill order. Linux only.
- Searchable conversation history: agents can search and re-read past conversations through an API documented in their system prompt. One request searches every past session and comes back with dated, topic-labelled snippets, each carrying a handle for pulling up the exchange around it; another replays a whole session at the level of detail asked for: just the incoming messages, the conversation without thinking traces, or everything including tool calls. Search matches the text as it reads on screen, so a phrase containing quotes or line breaks is found the way you would type it, and it takes regular expressions. An agent can read any agent in the rooms it can reach, not only itself, so one can pick up where another left off or recover work from before a /clear
...
### Other
- Voice-to-text prompting and text-to-speech responses (works locally; requires HTTPS via Tailscale for remote). Spoken punctuation is typed as punctuation: say "question mark", "comma", "period", "new line", and so on. Both use your preferred language, falling back to your browser's.
- Per-user profiles - your notification preferences, credentials, and personal preferences (the language your agents reply in, the Slide Mode toggle) follow you wherever you log in from
- Expand any prompt or memory field to a full-screen editor with the button in its corner, then Esc back to the dialog you were in
