...
- Agents use the browser: Agents can open pages, read, click, fill forms and attach a server file to a file input. Members pair desktop Chrome in Settings → You → Browser Use. The downloadable Chrome extension pairs with a short-lived code and lets members offer an open tab to an agent with Allow agent control in the popup. The agent reads and navigates that tab. Scoped reads reach individual feed entries and frames; snapshots include editable composer text. Each new offer defaults to Never expiring. The popup also offers 15 minutes, 1 hour or 4 hours and shows the local expiry time; agent actions do not extend it. Turning control off leaves pages/music running. The popup can also disconnect or unpair. Preview cards still run on the server for public and office-hosted pages.
