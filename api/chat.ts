...
- Agents use the browser: Agents can open pages, read, click, fill forms and attach a server file to a file input. Members select Server browser or Desktop Chrome in Settings → You → Browser Use. The downloadable Chrome extension pairs with a short-lived code and lets members offer an open tab to an agent with Allow agent control in the popup. The agent reads and navigates that tab. Each new offer defaults to Never expiring. The popup also offers 15 minutes, 1 hour or 4 hours and shows the local expiry time; agent actions do not extend it. Turning control off leaves pages/music running. The popup can also disconnect or unpair. Chrome mode has no remote Browser panel and never falls back while offline. Server browser mode keeps one profile per member and shares its logins with their agents. Its experimental Browser panel is off by default; an owner can enable it in Office Settings. Room members can watch; the manager can interact. Headless pages close after 15 idle minutes unless the manager is watching. Preview cards still run on the server.
