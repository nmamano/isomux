...
- Personal API tokens: drive the office from an external tool and drain durable replies from agents while the web UI shows the work and messages live. The raw token is shown once; you can review its approximate last authenticated request and revoke it at any time. Every token retains replies until the client sends ackThrough. Clients use sequence to detect duplicates; without acknowledgements, replies repeat and new sends fail with inbox_full at 100 retained messages. Message sends and drains accept Idempotency-Key for retries within a five-minute in-memory cache; API-token sends reject clientMessageId.
