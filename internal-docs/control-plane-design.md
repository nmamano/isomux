...
office DNS operation replaces every A record at the exact office and wildcard
names with one unproxied record for the current box. Deprovisioning deletes and
authoritatively re-lists those same two A record sets. It leaves every other
record type and name untouched.

The box installs the returned chain only after the names and key match and after
a read as the Caddy user succeeds. It writes the Caddyfile in the same directory,
...
The carry was deleted per Nil's ruling; rebuilds always take fresh issuance.
...
returns the stale chain. Every rebuild takes this path and handles no TLS
private-key material on the control plane.
