...
- `bun run test:opencode` - enables `ISOMUX_TEST_OPENCODE=1` for real-server first replies, provider-error scrubbing, credential scrubbing in Connections guidance and agent logs, and the adapter HTTP/SSE, permission, and history contracts. These checks use local provider mocks and spend no model credits. The adapter runs two ordered three-step scenarios on one lazy shared server; run all steps together (`-t "real OC1"` selects both scenarios). A failed step gives later steps an explicit missing-precondition error. The command also runs the default adapter and DI cases. Default `bun test` keeps the deterministic reply contract and skips the real-server scenarios and the DI credential-scrub case.
