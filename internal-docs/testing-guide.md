...
- `bun run test:opencode` - enables `ISOMUX_TEST_OPENCODE=1` for real-server first replies, provider-error scrubbing, credential scrubbing in Connections guidance and agent logs, and the adapter HTTP/SSE, permission, and history contracts. These checks use local provider mocks and spend no model credits. The adapter runs two ordered three-step scenarios on one lazy shared server; run all steps together (`-t "real OC1"` selects both scenarios). A failed step gives later steps an explicit missing-precondition error. The command also runs the default adapter and DI cases, and the supervisor case that does two real OpenCode startups ("refuses cross-process adoption when the environment revision changed"). Default `bun test` keeps the deterministic reply contract and skips the real-server scenarios and the DI credential-scrub case.
...
family are the worked examples. happy-dom carries a real xterm.js terminal
too, which is what makes `ui/log-view/TerminalPanel.replay.dom.test.tsx`
possible: it parses escape sequences and answers queries exactly as the
browser does, so the PTY input a mount produces is a real measurement
(1.6 s for that file, 2026-09-05).
...
- **Claude sign-in signals** - `ProviderSignInCard.dom.test.tsx` covers callback errors before and after a successful account push, and keeps sign-out failures visible. `ProviderSignInPush.dom.test.tsx` checks the connected chat card's new-conversation action. `provider-auth-affordance.di.test.ts` checks the old session environment after personal-account activation, the clear guidance, and the new environment after clearing. `provider-account-manager.test.ts` checks that a rejected duplicate callback does not prevent a clean success snapshot. `claude-session-root.test.ts` checks legacy root selection, stamp-once persistence and private metadata. The provider-auth seam also checks launch/init ordering, resume and cwd moves; the cron seam checks resumed run roots. `backends/claude/session-store.test.ts` drives real SDK reads and forks against scratch files and verifies that the store is consulted and the current root receives no fork.
...
- **Container updater** - `deploy/container/update-helper.test.ts` drives the Python recording-runner cases. `server/container-update-socket.test.ts` checks a real Unix socket and Python peer. `scripts/container-update-fixture.test.ts` checks the acceptance client against a real HTTP listener. `scripts/update-container.test.ts` rejects image revision and running-version mismatches. `deploy/install-container.test.ts` checks installation and post-update records; the office update route tests check owner authorization. `scripts/prepare-container-update-fixture.py`, `verify-container-update-root.sh`, and `check-container-update-fixture.py` exercise the real root service and container replacement only with reviewed root permission. See `container-updater-design.md` for the excluded installer stages and local-artifact substitution.
...
| Agent browser (browser) | T0/T1 | `browser-actions.test.ts`: action validation and screenshot provenance; `browser-upload.test.ts`: bounded server file payloads. Extension bridge/store/service/transport tests: current ownership, migration, pairing, generation, expiry, popup chains, timeout retention and settlement with no replay. REST (`routes-browser-extension.test.ts`, `routes-agent-affordances-rest.test.ts`): pairing, removed selection route, action validation, access and cross-agent rejection. `projection.test.ts`: retired panel commands have no effect. `preview-capture.test.ts` and app preview tests retain server screenshot coverage. ROUTE CAPS: `routes-table.test.ts`. | `browser-extension-office.live.test.ts`: isolated packaged raw Chrome, explicit offers, uploads, inactive clicks, timeout recovery and Off |
