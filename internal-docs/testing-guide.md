...
| isomux-memory (filesystem cross-agent memory) | T0/T1 | STORE/RENDER (`memory-store.test.ts`): line round-trip, supersede/tombstone resolution, dedup matching, and per-scope size caps + trim notice — caps are a render/auto-load concern, `read()`/REST GET are intentionally UNCAPPED. REST contract (`routes-memory-rest.test.ts`): auth wall, all scopes, supersede/tombstone by id, dedup 409 with matched id, and human-curation raw read + rewrite. PROMPT (`system-prompt.test.ts`): auto-load attributed layer + no-boss-path. ROUTE CAPS (`routes-table.test.ts`). NOTE: the verbatim curation raw routes are PER-SURFACE and permission-inheriting (`/api/office/memory/raw` owner-only, `/api/rooms/:id/memory/raw` room-gated, `/api/agents/:id/memory/raw` agent-gated, `/api/users/:name/memory/raw` self-or-owner) — NOT the general permissive `/api/memory` surface. | Playwright curation smoke DEFERRED (no e2e infra in repo; behavior covered at T0/T1) — manual UI verification + a future opt-in browser smoke |
