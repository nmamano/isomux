...
| Agent browser (browser) | T0/T1 | `browser-session.test.ts`: the BrowserPool seam with a stub browser - validation matrix (unknown action, missing field, non-http URL, credentials in URL, viewport range), one browser and one context per agent, per-member storage-state restore/isolation and merge-on-close, per-agent serialization (two cold calls make one context; a close never lands inside an action), office-wide lifecycle serialization (agent A's close cannot strand agent B inside newContext on a closed browser), the timeout drain (a late operation settles before the caller is answered), no_page checked before Playwright, one page per agent when a window opens, idle close, close-with-the-last-context, relaunch after a disconnect, no_browser/launch_failed/action_failed/no_page codes, the text and snapshot caps, and the query-stripped screenshot caption. REST (`routes-agent-affordances-rest.test.ts`): the route's gate, its 400s, the close no-op, launch_failed through `ISOMUX_PREVIEW_BROWSER`, and cross-agent 403. ROUTE CAPS: `routes-table.test.ts`. | driving real Chrome (needs Chrome on the box) |
