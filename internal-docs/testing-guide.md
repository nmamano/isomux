...
- **Queue / fork / usage** — the message queue and coalescing, conversation branching, and usage accounting, plus the `/isomux-usage` report's room scoping (a member sees only their rooms; cron jobs are owner-only). Tests: `queue.test.ts`, `fork-usage.test.ts`, `usage-scoping.test.ts`.
