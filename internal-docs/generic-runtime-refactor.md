...
- Slice 4 (PENDING, the cut): the inbound `presence_update` server logic switches to read `currentRoomId`, and the dense `room`/`currentRoom` are removed from the wire (`AgentInfo.room`, `PresenceInfo.currentRoom`, and the inbound `presence_update.currentRoom`). The per-recipient dense-index projection on the room axis goes away. This is the one restart-requiring, client-coordinated step; by then both clients already read ids, so the cut is a deletion, not a behavior change. Folds in the deferred slice-2/3 server cleanups (e.g. `buildPresenceListFor` reusing `projection.globalRoomIdToIndex`) and exposes a client-ready close capability on `RoomWire` (ideally a `canClose`/`deletable` flag reflecting the full server rule — protected-first-room + empty + access; at minimum a protected/canonical-room id with emptiness kept client-local) so the client delete affordance can drop its remaining visible-index gate (`roomIndex > 0` in `RoomSettingsModal`, `i > 0` in `RoomTabBar`), which is only a default-order-compatible approximation and is not correct under a custom per-user view order.
