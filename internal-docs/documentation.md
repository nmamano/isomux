...
- **Pages (in sidebar/nav order):**
  ...
  - `self-hosted.md` - the one always-on-server page. Covers the Render Blueprint (`render.yaml`, with the BuildKit Dockerfile in `deploy/render/`), the unattended VPS install (`deploy/install.sh`: fresh Ubuntu 24.04 → HTTPS-served office + owner invite link; keep the parameter table in sync with the script's env vars), the own-hardware walkthrough (keep it running / make it reachable - the Tailscale Funnel agent prompt lives here, nowhere else - / authorize users), backups, out-of-memory protection, and the deployment-boundary matrix (reachability, app addresses, firewall ownership, request logging, exclusions). The retired `/docs/vps-install` and `/docs/deployments` URLs redirect here (`vercel.json`).
