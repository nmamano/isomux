...
- `docs/developer-api.md` also documents the discovery manifest: a desk agent has a 1-based `room`; the receptionist has `room: null`, `roomName: "Lobby"`, and `roomId: "lobby"`. Keep both discovery aliases and the members-chat routes in sync with the runtime.
- `server/system-prompt.ts` `buildReceptionistSystemPrompt()` - the receptionist's prompt: the `ISOMUX_KNOWLEDGE` block of `api/chat.ts` (section 3) plus the office's name, members and office-wide instructions. A feature change edits the knowledge once and reaches both the site chatbot and the receptionist. The receptionist's record lives in `<state root>/receptionist.json`, outside `agents.json`.
