...
- **Pages (in sidebar/nav order):**
  ...
  - `safety-hooks.md` - maintained scope, evidence, and limits for Claude and Codex pre-tool safety hooks. Update it when the policy surface, backend hook contract, installation checks, failure behavior, or isolation boundary changes.
