...
it("uses one plain-text, code-point-limited excerpt rule for text and files", () => {
  ...
  expect(membersChatExcerpt(" \n", [{ originalName: "one.pdf" }, { originalName: "two.png" }])).toBe("one.pdf, two.png");
  expect(membersChatExcerpt("text", [{ originalName: "ignored" }])).toBe("text");
});

for (const [source, expected] of [
  ["**Thanks!**", "Thanks!"],
  ["**bold _and italic_** [label](https://example.com)", "bold and italic label"],
