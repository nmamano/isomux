...
const frame = {
  ...
  width: 1280,
  height: 800,
  jpeg: new Uint8Array([255, 216, 12, 255, 217]),
};
...
test("rejects truncated identities, payloads, foreign binary protocols and versions", () => {
  ...
  for (let n = 0; n < encoded.length; n++)
    expect(decodeBrowserFrame(encoded.slice(0, n))).toBeNull();
  for (const offset of [0, 4, 5, 6, 7, 24]) {
    const bad = encoded.slice();
    bad[offset] ^= 2;
    expect(decodeBrowserFrame(bad)).toBeNull();
  }
  const badId = encoded.slice();
  badId[28] = 255;
  expect(decodeBrowserFrame(badId)).toBeNull();
  const badGeneration = encoded.slice();
  new DataView(badGeneration.buffer).setFloat64(8, NaN);
