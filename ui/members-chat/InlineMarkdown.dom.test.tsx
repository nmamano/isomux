...
it("renders only bold, italics and safe web links", () => {
  const view = render(createElement(InlineMarkdown, {
    content: '**bold *and italic*** _italic_ https://example.com [site](https://example.org)',
  }));
  expect(view.container.querySelector('strong')?.textContent).toBe('bold and italic');
  expect(view.container.querySelectorAll('em').length).toBe(2);
  const links = view.getAllByRole('link');
  expect(links.map((link) => link.getAttribute('href'))).toEqual(['https://example.com', 'https://example.org']);
...
it("keeps unsupported syntax and hostile markup as text", () => {
  ...
  expect(view.container.querySelector('h1,pre,code,del,img,a,script') === null).toBe(true);
...
for (const [content, html] of [
  ...
  ['[mail](mailto:hello@example.com)', link('mailto:hello@example.com', 'mail')],
  ['first\nsecond', 'first\nsecond'],
...
it('keeps a quote in a destination inside the href attribute', () => {
  const view = render(createElement(InlineMarkdown, { content: '[x](https://example.com/"onmouseover="alert)' }));
  const anchor = view.getByRole('link');
  expect(anchor.getAttribute('href')).toBe('https://example.com/"onmouseover="alert');
