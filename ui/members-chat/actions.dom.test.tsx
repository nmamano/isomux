...
const messages = [1, 2].map((n) => ({ id: `202609-0000000${n}`, kind: "user" as const, userId: "u1", userName: "Tester", content: `message ${n}`, attachments: [], timestamp: n }));

it("closes an outside menu, closes Edit, and closes confirmed Delete even when its request fails", async () => {
  setApiShim(async () => { throw new ApiError(500, "failed", ""); });
  const view = render(onLanguage("en", createElement(MembersChatPanel), { membersChat: { messages, loaded: true, hasMore: false, readPointer: messages[1].id, unread: 0 } }));
  ...
  await act(async () => fireEvent.click(view.getByTitle('Click again to delete')));
