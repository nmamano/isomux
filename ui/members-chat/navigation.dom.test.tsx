...
const { StateCtx, StoreProvider, useAppState, useDispatch } = await import("../store.tsx");
...
function Content() {
  ...
  return createElement(RoomTabBar, { membersChatLoadFailed: loadFailed, onRetryMembersChat: retry });
}
function Harness() {
  const state = useAppState();
  const send = useDispatch();
  useEffect(() => { dispatch = send; }, [send]);
  return createElement(StateCtx.Provider, { value: { ...state, isMobile: mobile } },
    createElement(LanguageProvider, null, createElement(Content)));
}
setShim(() => {}, () => {});
afterAll(() => { setApiShim(null); setShim(() => {}, () => {}); connect(() => {}, () => {}); });
const message = { id: "202609-00000001", kind: "user" as const, userId: "other", userName: "Sam", content: "Hello", attachments: [], timestamp: 1 };
it("hydrates the closed chat, moves its dot to the mobile entry, clears on a read pointer, and closes back to the scene", async () => {
  ...
  setApiShim(async (method, path, body) => {
    if (method === "GET" && path.startsWith("/api/members-chat")) {
      pages++;
      return { messages: [message], hasMore: false, readPointer: null, unread: 1 };
    }
    if (path === "/api/members-chat/read") { reads.push(body); return { readPointer: message.id, unread: 0 }; }
    return {};
  });
  ...
  expect(lobby.querySelector("[data-lobby-unread]")?.getAttribute("aria-label")).toBe("Unread message: 1");
  ...
  expect(pages).toBe(1);
  act(() => dispatch({ type: "members_chat_read", readPointer: message.id, unread: 0 }));
  ...
  expect(view.queryByRole("dialog")).toBeNull();
  act(() => dispatch({ type: "members_chat_message", message: { ...message, id: "202609-00000002" } }));
  expect(view.getByRole("img", { name: "Unread message: 1" })).not.toBeNull();
  expect(reads).toEqual([]);
  act(() => dispatch({ type: "full_state", agents: [], recentCwds: [],
    office: { prompt: null, name: null }, rooms: [], killedAgents: [] }));
  await act(async () => {});
  expect(pages).toBe(2);
  expect(reads).toEqual([]);
});
it("does not hydrate when disabled, and contains a load failure", async () => {
  ...
  mobile = true;
  let calls = 0;
  setApiShim(async () => { calls++; throw new Error("offline"); });
  const view = render(createElement(StoreProvider, null, createElement(Harness)));
  await act(async () => {});
  expect(calls).toBe(0);
  enabled = true;
  ...
  expect(view.getByRole("alert").textContent).toContain("Could not load the chat");
  setApiShim(async () => { calls++; return { messages: [], hasMore: false, readPointer: null, unread: 0 }; });
