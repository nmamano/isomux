...
function MobileChatPanel({ loadFailed, onRetry, onClose, entryRef }: {
  loadFailed: boolean; onRetry?: () => void; onClose: () => void; entryRef: RefObject<HTMLButtonElement | null>;
}) {
  const { t } = useI18n();
  ...
  return <div ref={panelRef} role="dialog" aria-modal="true" tabIndex={-1}
    ...
    style={{ position: "fixed", inset: 0, width: "100%", height: "100dvh",
      zIndex: 1000, background: "var(--bg-base)", color: "var(--text-primary)" }}>
    <MembersChatPanel onClose={onClose} loadFailed={loadFailed} onRetry={onRetry} />
  </div>;
}

// Mounted only on the mobile lobby. Leaving the lobby also forgets open state.
export function LobbyChat({ loadFailed, onRetry }: { loadFailed: boolean; onRetry?: () => void }) {
  const { t } = useI18n();
  ...
  return <>
    <button ref={entryRef} onClick={() => setOpen(true)}
      style={{ position: "relative", flexShrink: 0, padding: "4px 12px",
    ...
    {open && <Portal><MobileChatPanel entryRef={entryRef} loadFailed={loadFailed} onRetry={onRetry} onClose={close} /></Portal>}
  </>;
}
