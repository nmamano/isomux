...
const original = { id: "202609-00000001", kind: "user" as const, userId: "u1", userName: "Tester", content: "**original**", attachments: [], timestamp: 1 };
const reply = { ...original, id: "202609-00000002", content: "reply", replyTo: { id: original.id, userName: "Tester", excerpt: "**original**" } };
function panel(language: "en" | "es" | "ca" = "en") {
  return onLanguage(language, createElement(MembersChatPanel), { isMobile: true, membersChat: { messages: [original, reply], loaded: true, hasMore: false, readPointer: reply.id, unread: 0 } });
}
...
it("quotes plain snapshot text, jumps to a loaded target, sends only its id and cancels a quote", async () => {
  ...
  const anchor = view.container.querySelector<HTMLElement>(`#members-chat-${original.id}`)!;
  let scrolled = false;
  anchor.scrollIntoView = () => { scrolled = true; };
  const quote = view.container.querySelector<HTMLButtonElement>('[data-members-chat-quote]')!;
  ...
  expect(view.container.querySelector('[data-members-chat-composer-quote]')?.textContent).toContain('Testeroriginal');
  fireEvent.change(composer, { target: { value: 'my draft' } });
  await act(async () => fireEvent.click(view.getByRole('button', { name: 'Send' })));
  expect(requests).toEqual([{ text: 'my draft', attachments: [], replyTo: original.id }]);
...
for (const [language, replyLabel, error] of [
  ['en', 'Reply', 'The original message was deleted. Cancel the reply or choose another message.'],
  ['es', 'Responder', 'El mensaje original se ha eliminado. Cancela la respuesta o elige otro mensaje.'],
  ['ca', 'Respon', 'El missatge original s’ha suprimit. Cancel·la la resposta o tria un altre missatge.'],
] as const) {
  it(`retains a draft after a deleted target with ${language} copy`, async () => {
    setApiShim(async () => { throw new ApiError(404, 'reply_not_found', ''); });
    ...
    expect(view.container.querySelector('[data-members-chat-composer-quote]') !== null).toBe(true);
  });
}
