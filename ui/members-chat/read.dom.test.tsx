...
const message = (id: string) => ({ id, kind: "user" as const, userId: "other", userName: "Sam", content: id, attachments: [], timestamp: 1 });
const settleRead = () => act(async () => { await new Promise(resolve => setTimeout(resolve, 550)); });
it("marks a short visible list read, pauses in history and hidden tabs, and resumes at the bottom", async () => {
  ...
  setApiShim(async (_method, path, body) => {
    if (path === "/api/members-chat/read") reads.push((body as { lastReadId: string }).lastReadId);
  ...
  function panel(id: string, mobile = false) {
    return onLanguage("en", createElement(MembersChatPanel), {
      isMobile: mobile,
      membersChat: { loaded: true, messages: [message(id)], hasMore: false, unread: 1, readPointer: null },
    });
  }
