...
it("keeps ghost nodes mounted while authoritative seats change", () => {
  ...
  const scene = (spot: string) => createElement(LobbyScene, { rooms: [], officeName: null, mode: "dark", layout: "nilo", presences: [{ ...p, lobbySpotId: spot }] });
...
for (const [lang, label] of [["es", "Moverse aquí"], ["ca", "Mou-te aquí"]] as const) {
  it(`labels free spots in ${lang}`, () => {
    const view = render(onLanguage(lang, createElement(LobbyScene, { rooms: [], officeName: null, mode: "dark", layout: "nilo", onMoveGhost: () => {} })));
...
it("keeps overflow body identification while suppressing its name chip", () => {
  ...
  const view = render(createElement(LobbyScene, { rooms: [], officeName: null, mode: "dark", layout: "nilo", presences: [p] }));
  expect(view.container.querySelectorAll('div[title="Waiting viewer"]')).toHaveLength(1);
