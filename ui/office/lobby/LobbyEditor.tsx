...
export function LobbyEditor({
  ...
  function startDrag(
    ...
    target: { kind: "item"; key: number } | { kind: "recep" },
  ) {
    e.preventDefault();
    e.stopPropagation();
    const startX = e.clientX,
      startY = e.clientY;
    const item =
      target.kind === "item" ? items.find((it) => it.key === target.key) : null;
    const start =
      target.kind === "item"
        ? { a: item!.a, b: item!.b, h: item!.h ?? 60 }
        : { a: recep.a, b: recep.b, h: 0 };
    ...
    const move = (ev: PointerEvent) => {
      ...
      if (target.kind === "recep") {
        setRecep({
          a: start.a + (dx / COL.dx + dy / COL.dy) / 2,
          b: start.b + (dy / ROW.dy - dx / COL.dx) / 2,
        });
        return;
      }
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === "Delete" || e.key === "Backspace") {
        ...
        update(
          selected,
          it.wall ? { h: (it.h ?? 60) + step * 10 } : { b: it.b - step },
        );
    ...
    try {
      const res = await fetch("/save", {
        ...
        body: JSON.stringify({
          ...
          mode,
          receptionist: recep,
          placements: items.map(({ key: _k, ...p }) => p),
          ts: exportedTs,
        }),
      });
      const j = (await res.json()) as { path?: string };
      setStatus(
        res.ok ? `saved as ${saveName}` : `save failed: ${JSON.stringify(j)}`,
      );
    ...
    <div style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
      ...
      <div style={panelStyle}>
        ...
        >
          <select
            value={layout}
            onChange={(e) => loadLayout(e.target.value as LobbyLayoutId)}
          >
        ...
        <div style={{ color: "var(--text-dim)", marginBottom: 8 }}>
          Drag the dots. Click one, then arrows nudge (shift: bigger), Delete
          removes. Wall props slide along their wall and up/down. Yellow: floor,
          blue: wall, green: receptionist. Row boxes are wall, height, scale, z
          (draw order: raise it to pass in front of a neighbour) and facing. NE
          and NW turn a piece around, so you see its back.
        </div>
        <div
          style={{
            display: "flex",
        ...
        >
          <select
            ...
            onChange={(e) => {
              ...
              setAddVariant(
                LOBBY_PROPS.find((f) => f.id === e.target.value)!.variants[0]
                  .id,
              );
            }}
          >
                ...
                >
                  <td style={{ padding: 2 }}>
                    <select
                      value={it.family}
                      onChange={(e) => {
                        const f = LOBBY_PROPS.find(
                          (x) => x.id === e.target.value,
                        )!;
                        update(it.key, {
                          family: f.id,
                          variant: f.variants[0].id,
                        });
                      }}
                    >
                      {LOBBY_PROPS.map((f) => (
                  ...
                  <td style={{ padding: 2, whiteSpace: "nowrap" }}>
                    ...
                    {it.wall && (
                      <input
                        ...
                        value={round(it.h ?? 60)}
                        onChange={(e) =>
                          update(it.key, { h: Number(e.target.value) })
                        }
                        title="height"
                      />
                    )}
                    <input
                      ...
                      value={it.scale ?? 1}
                      onChange={(e) =>
                        update(it.key, { scale: Number(e.target.value) })
                      }
                      title="scale"
                    />
                    {!it.wall && (
                      <input
                        ...
                        value={it.z ?? 0}
                        onChange={(e) =>
                          update(it.key, { z: Number(e.target.value) })
                        }
                        title="draw order: higher is drawn later, in front"
                      />
                    )}
                    {it.wall ? (
                      <label title="flip">
                        <input
                          ...
                          checked={!!it.flip}
                          onChange={(e) =>
                            update(it.key, { flip: e.target.checked })
                          }
                        />
                        f
                      </label>
                      ...
                      (() => {
                        ...
                        const n = variantFacings(
                          fam.variants.find((v) => v.id === it.variant) ??
                            fam.variants[0],
                        );
                        if (n === 1) return null;
                        const options =
                          n === 4 ? ["SE", "SW", "NE", "NW"] : ["SE", "SW"];
                        return (
                          <select
                            value={it.facing ?? (it.flip ? "SW" : "SE")}
                            onChange={(e) =>
                              update(it.key, {
                                facing: e.target.value as Placement["facing"],
                                flip: undefined,
                              })
                            }
                            title={
                              n === 4
                                ? "which way it faces; NE and NW show it from behind"
                                : "which way it faces"
                            }
                          >
                            {options.map((o) => (
                    ...
                    <button
                      onClick={() =>
                        setItems((l) => l.filter((x) => x.key !== it.key))
                      }
                    >
                      x
                    </button>
        ...
        >
          ...
          <button
            onClick={() => setDraft(exportedTs)}
            title="the same layout as a layouts.ts block, to copy"
          >
            show as TS
          </button>
        </div>
