...
const ghost = (
  ...
  avatarColor: "#123456",
  focusedAgentId: "receptionist",
  viewMode: "office",
});
it("pins stable server ids to the saved layout", () => {
  expect(LOBBY_LAYOUTS.nilo.ghostSpots.map((s) => s.id)).toEqual([
    ...LOBBY_SPOT_IDS,
  ]);
});
it("places self and peers at named seats regardless of focus, with unique overflow positions", () => {
  ...
  const input = [
    ghost("a", LOBBY_SPOT_IDS[0]),
    ghost("b", LOBBY_SPOT_IDS[1]),
    ghost("c", null),
    ghost("d", null),
    { ...ghost("e", null), currentRoomId: "ordinary" },
  ];
  const rows = lobbyGhostPlacements(input, spots);
  expect(rows.map((p) => p.presence.connectionId)).toEqual([
    "a",
    "b",
    "c",
    "d",
  ...
  const moved = lobbyGhostPlacements(
    [{ ...input[0], lobbySpotId: LOBBY_SPOT_IDS[1] }],
    spots,
  );
...
it("wraps 18 viewers within the scene and keeps larger overflow lines within its bounds", () => {
  for (const count of [18, 40]) {
    const viewers = Array.from({ length: count }, (_, i) =>
      ghost(String(i).padStart(2, "0"), LOBBY_SPOT_IDS[i] ?? null),
    );
    const rows = lobbyGhostPlacements(viewers, LOBBY_LAYOUTS.nilo.ghostSpots);
    expect(rows).toHaveLength(count);
    expect(new Set(rows.map((p) => `${p.left}:${p.top}`)).size).toBe(
      Math.min(count, 18),
    );
...
it("keeps overflow spacing fixed when later viewers join", () => {
  const viewers = Array.from({ length: 18 }, (_, i) =>
    ghost(String(i).padStart(2, "0"), LOBBY_SPOT_IDS[i] ?? null),
  );
  const before = lobbyGhostPlacements(viewers, LOBBY_LAYOUTS.nilo.ghostSpots);
  const after = lobbyGhostPlacements(
    [
      ...viewers,
      ...Array.from({ length: 22 }, (_, i) => ghost(String(i + 18), null)),
    ],
    LOBBY_LAYOUTS.nilo.ghostSpots,
  );
