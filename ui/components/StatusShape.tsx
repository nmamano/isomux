...
export function StatusShape({ kind, rotate = 0 }: {
  kind: "triangle" | "dot" | "check";
  rotate?: number;
}) {
  // Same CSS circle as AppsView's StateDot, inheriting the surrounding color.
  if (kind === "dot") return <span aria-hidden="true" style={{ display: "inline-block", width: 8, height: 8, borderRadius: "50%", flexShrink: 0, background: "currentColor" }} />;
  return (
    <svg aria-hidden="true" width="1em" height="1em" viewBox="0 0 12 12"
      style={{ display: "inline-block", verticalAlign: "-0.1em", flexShrink: 0, transform: `rotate(${rotate}deg)` }}
