...
it("keeps the same action buttons through pending, successful, and failed lifecycle requests", async () => {
  const app = {
    name: "controls-fixture", command: "bun run start", cwd: "/fixture",
    dataDir: "/fixture/data", port: 21000, hostLabel: "controls-fixture",
    hostGen: 1, userId: "owner", username: "Owner", createdBy: "Owner",
    createdAt: 1, state: "running" as const, restartCount: 0,
  };
  let settle!: (value: unknown) => void;
  let reject!: (reason: unknown) => void;
  const calls: string[] = [];
  setApiShim(async (method, path) => {
    if (method === "GET" && path === "/api/apps") return [{ ...app, canManage: true }];
    if (method === "POST") {
      calls.push(path);
  ...
  const view = render(<StoreProvider><AppsView onClose={() => {}} /></StoreProvider>);
  await act(async () => {});
  const card = view.getByRole("link", { name: app.name }).parentElement!.parentElement!;
  ...
  const sameButtons = () => {
    ...
    buttons.forEach((button, index) => expect(current[index] === button).toBe(true));
  };
  for (const [index, verb, state] of [
    [1, "stop", "stopped"], [0, "start", "running"], [2, "restart", "running"],
  ] as const) {
    ...
    expect(buttons.map((button) => button.disabled)).toEqual(
      state === "stopped" ? [false, true, true, false, false] : [true, false, false, false, false],
    );
  }
  await act(async () => buttons[1].click());
  const failure = "fixture supervisor failure";
  await act(async () => reject(new ApiError(500, "supervisor_failed", failure)));
  sameButtons();
