...
test("unavailable selection stays explicit; pairing, replacement and revoke use self routes", async () => {
  let status = { backend: null as string | null, selectionRequired: true, paired: false, online: false, member: { id: "self", name: "Fixture member" }, version: "0.1.0" };
  const writes: unknown[] = [];
  setApiShim(async (method, path, body) => {
    if (method === "GET") return status;
    writes.push([method, path, body]);
    if (method === "PATCH") status = { ...status, backend: (body as { backend: string }).backend, selectionRequired: false };
    if (method === "DELETE") status = { ...status, paired: false, online: false };
    if (method === "POST") return { code: "fixture-code", expiresAt: Date.now() + 300000 };
  });
  const view = render(<BrowserPane />);
  await act(async () => {});
  expect((view.getByTestId("browser-backend") as HTMLSelectElement).value).toBe("");
  expect((view.getByTestId("browser-pair") as HTMLButtonElement).disabled).toBe(true);
  expect(writes).toHaveLength(0);
  await act(async () => fireEvent.change(view.getByTestId("browser-backend"), { target: { value: "extension" } }));
  await act(async () => fireEvent.click(view.getByTestId("browser-pair")));
  expect(writes).toEqual([["PATCH", "/api/me/browser", { backend: "extension" }], ["POST", "/api/me/browser/pair", { replace: false }]]);
  expect((view.getByTestId("browser-code") as HTMLInputElement).value).toBe("fixture-code");
  view.unmount();
  ...
  expect(writes.at(-1)).toEqual(["POST", "/api/me/browser/pair", { replace: true }]);
