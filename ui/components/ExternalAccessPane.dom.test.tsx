...
it("shows the hosted address with no URL input, toggle or save", async () => {
  setApiShim(async () => ({
    hosted: true, externalAccess: true,
    publicOrigin: "https://office.isomux.app", envOriginSet: false,
    envOrigin: null, boundLoopback: false,
  }));
  const view = render(onLanguage(null, <ExternalAccessPane />));
  await act(async () => {});
  expect(view.getByText("https://office.isomux.app")).toBeTruthy();
  expect(view.getByText("Isomux manages this office’s address; it cannot be changed here.")).toBeTruthy();
...
it("keeps self-hosted controls and the restart instructions after a save", async () => {
  setApiShim(async (method) => method === "GET" ? {
    hosted: false, externalAccess: true,
    publicOrigin: "https://office.example", envOriginSet: false,
    envOrigin: null, boundLoopback: false,
