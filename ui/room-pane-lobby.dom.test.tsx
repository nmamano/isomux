...
describe("room settings memory scopes", () => {
  for (const type of ["lobby", "office"] as const) {
    it(`${type} exposes only supported memory and saves settings`, async () => {
      calls.length = 0;
      const room = {id: type, name: type, type, prompt: null, canCloseWhenEmpty: false};
      const view = render(onLanguage(null, createElement(RoomPane, {roomId: type, onDeleted() {}}), {rooms: [room]}));
      await act(async () => {});
      expect(calls.some((call) => call.startsWith("GET /api/memory"))).toBe(type === "office");
      expect(view.queryAllByRole("textbox").length).toBe(type === "office" ? 3 : 1);
