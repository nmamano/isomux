...
A `*.dom.test.tsx` file mounts a React component in happy-dom. Read the render
test rules in `internal-docs/testing-guide.md` before you write or edit one. The
file shape is load-bearing: a static import of UI code evaluates it before the
DOM exists, which can pass and record the wrong state rather than fail.
...
- `site/` - Landing page and demo, deployed to isomux.com via Vercel. `bun run build:demo` builds the office and standalone app entries from `ui/demo-entry.tsx`, `ui/demo-app-entry.tsx`, and their HTML templates into `site/demo/`. `site/docs/**` and the generated files under `site/demo/` are build outputs (gitignored, regenerated by `bun run build:docs` and `bun run build:demo`) - editing them directly looks like it works, then vanishes on the next build. Edit `scripts/build-docs.ts`, `ui/demo.html`, and `ui/demo-app.html` instead.
