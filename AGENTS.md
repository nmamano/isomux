...
- Agent-built apps run as systemd **user** units that isomux generates; `server/app-supervisor.ts` is the only runtime module that manages those units. The registry owns names and ports and never recycles either, so a deleted app's name and port stay retired.
